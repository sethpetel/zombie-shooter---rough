controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    if (player.image == gunLeft || player.image == gunShootLeft) {
        player.setImage(gunShootLeft)
        sprites.createProjectileFromSprite(bulletLeft, player, -400, 0)
    } else {
        player.setImage(gunShootRight)
        sprites.createProjectileFromSprite(bulletRight, player, 400, 0)
    }
})
...
gunShootLeft = img`
    ...
    . . . . . . . . . f f f . . . . 
    `
gunRight = gunLeft.clone()
gunRight.flipX()
gunShootRight = gunShootLeft.clone()
gunShootRight.flipX()
player = sprites.create(gunRight, SpriteKind.Player)
controller.moveSprite(player, 120)
...
function zombieSpawn(zombies: any[]){
    for(let i = 0; i < zombies.length; i++){
        ...
        for (let j = 0; j < waveNumber; j++){
            ...
            tempZombie.follow(player, zombieSpeed)
